...
export const BookingsPage: React.FC = () => {
  ...
  const loadBookings = async () => {
    try {
      const userBookings = await database.getBookingsByUser(user!.id);
      setBookings(userBookings);

      // Load spot details for each booking
      const spotIds = [...new Set(userBookings.map(b => b.spotId))];
      const spotData: { [key: string]: ParkingSpot } = {};
      
      for (const spotId of spotIds) {
        const spot = await database.getParkingSpotById(spotId);
        if (spot) {
          spotData[spotId] = spot;
        }
      }
  ...
  const handleCancelBooking = async (bookingId: string) => {
    if (confirm('Are you sure you want to cancel this booking?')) {
      try {
        await database.updateBookingStatus(bookingId, 'cancelled');
    ...
    if (!spot) {
      return (
        <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
          <p className="text-gray-500">Loading spot details...</p>
        </div>
      );
    }
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <RatingReviewModal
          ...
          spotName={spots[bookings.find(b => b.id === showReviewModal)?.spotId || '']?.name || ''}
